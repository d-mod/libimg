 ...
 **/
import {VER_02} from "./constants/version"
import {ARGB_8888, LINK} from "./constants/color-bits"
import {COMPRESS_NONE, COMPRESS_ZLIB} from "./constants/compress-mode"
import {Sprite} from "./model/sprite"
import queues from "./queues"


export function merge(list = [], options = {targetVersion: 0}) {
    list = list.reverse()
    list.sort(compare)
    let count = 0
    let version = VER_02
    ...
    for (let i = 0; i < count; i++) {
        ...
        let x = 800, y = 600
        let colorBits = ARGB_8888
        let compressMode = COMPRESS_NONE
...
export function draw(width, height, list = []) {
